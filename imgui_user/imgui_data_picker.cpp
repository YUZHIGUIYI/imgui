...
namespace ImGui
{
    ...
    static void RecalculateDateDependentFields(tm& date)
    {
    ...
    static float CalcMaxPopupHeightFromItemCount(int items_count)
    {
        ...
        return (g.FontSize + g.Style.ItemSpacing.y) * static_cast<float>(items_count) - g.Style.ItemSpacing.y +
                        ...
                        const char* upArrowString, const char* downArrowString)
    {
        ...
        if(strlen(dayNames[0]) == 0)
        {
            ...
            for(int i = 0; i < 12; i++)
            {
                ...
                if(strlen(monthName) == 0)
                {
                    ...
                    monthName[0] = std::toupper(monthName[0]);
                // fprintf(stderr,"%s\n",monthNames[i]);
        ...
        const float widthRequiredByCalendar = 2.0f * arrowLeftWidth + 2.0f * arrowRightWidth +
                                                ImGui::CalcTextSize(monthNames[maxMonthWidthIndex]).x +
                                                ImGui::CalcTextSize("9999").x + 60.0f + widthAdder;
        ...
        for(int dw = 0; dw < 7; dw++)
        {
            ...
            if(dw == 0 || dw == 6)
            {
                ...
                ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(l * 2.0f > 1.0f ? 1.0f : l * 2.0f, l * 0.5f, l * 0.5f, tc.w));
            }
            ImGui::Separator();
            ImGui::AlignTextToFramePadding();
            ...
            if(dw != 6)
                ImGui::SameLine(ImGui::GetContentRegionAvail().x -
                                static_cast<float>(6 - dw) * (ImGui::GetContentRegionAvail().x / 7.0f));
        }
                            ...
                            const char* upArrowString, const char* downArrowString)
    {
        ...
        if(DateChooser("DateChooser##id", date, dateFormat, nullptr, leftArrow, rightArrow, upArrowString,
                        downArrowString))
        {
