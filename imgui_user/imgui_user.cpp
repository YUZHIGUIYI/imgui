...
namespace ImGui
{
    ...
    void LabelTextExV(const char* label, const char* fmt, va_list args)
    {
        ...
        // Render
        const char* value_text_begin = g.TempBuffer.begin();
        const char* value_text_end = value_text_begin + ImFormatStringV(g.TempBuffer.begin(), g.TempBuffer.Size, fmt, args);
